...
class TestJsonBuilderBackwardCompatibility(unittest.TestCase):
    def test_omitting_mmsplice_result_still_produces_a_complete_record(self):
        ...
        for key in ("variant", "sequence_context", "dna_model_results", "rna_analysis",
                    "protein_analysis", "alphamissense", "blast", "clinvar", "dbsnp",
    ...
    def test_passing_mmsplice_result_is_included_verbatim(self):
        ...
        result = build_variant_result(
            variant_dict={}, sequence_context={}, dna_model_results={}, rna_result={},
            protein_result={}, blast_result={}, clinvar_result={}, dbsnp_result={},
            interpretation={}, errors=[], mmsplice_result=mmsplice_result,
        )
...
class TestInterpretationEngineMMSpliceEvidence(unittest.TestCase):
    ...
    def test_no_mmsplice_result_does_not_change_existing_behavior(self):
        ...
        self.assertEqual(without["significance_score"], with_none["significance_score"])
    ...
    def test_mmsplice_evidence_is_additive_not_replacing(self):
        ...
        self.assertEqual(
            len(with_mmsplice["supporting_evidence"]), len(without_mmsplice["supporting_evidence"]) + 1
        )
        self.assertGreater(with_mmsplice["significance_score"], without_mmsplice["significance_score"])
    ...
    def test_mmsplice_weight_zero_excludes_from_score_but_keeps_evidence_text(self):
        ...
        self.assertEqual(with_mmsplice["significance_score"], without_mmsplice["significance_score"])
        self.assertEqual(
            len(with_mmsplice["supporting_evidence"]), len(without_mmsplice["supporting_evidence"]) + 1
        )
