...
class TestMMSpliceServiceEligibility(unittest.TestCase):
    def setUp(self):
        ...
        self.service = MMSpliceService(
            model=self.model, sequence_context_generator=self.seq_ctx, species="human"
        )
    ...
    def test_deep_intronic_variant_is_ineligible(self):
        ...
        with mock.patch.object(
            self.service, "_fetch_overlapping_exons",
...
class TestMMSpliceServiceEndToEndWithMocks(unittest.TestCase):
    ...
    def test_eligible_variant_calls_predictor_and_returns_full_schema(self):
        ...
        fake_raw = mock.Mock(
            delta_logit_psi=-3.0, donor_delta=-1.0, acceptor_delta=-3.0, exon_delta=-0.5,
            acceptor_intron_delta=0.0, donor_intron_delta=0.0,
            alt_donor_score=0.1, alt_acceptor_score=0.2,
            exon_skipping_score=0.0, intron_retention_score=0.0,
            model_version="mmsplice-2.4.0-test", runtime_ms=12.3,
        )

        with mock.patch.object(self.service, "_fetch_overlapping_exons", return_value=[exon]), \
...
class TestNullResultSchema(unittest.TestCase):
    def test_null_result_contains_every_required_key(self):
        required_keys = {
            "supported", "predicted", "delta_logit_psi", "donor_score", "acceptor_score",
            "exon_skipping", "intron_retention", "alt_donor", "alt_acceptor", "confidence",
            "interpretation", "runtime_ms", "model_version",
        }
