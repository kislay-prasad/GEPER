...
class VerifyModelArtifactTest(unittest.TestCase):
    ...
    def test_matching_bytes_verify_and_only_then_is_hash_only_claimed(self):
        with tempfile.TemporaryDirectory() as tmp:
            path, _ = self._blob(tmp, b"the real weights")
    ...
    def test_verification_routes_through_weightcache_verify_checksum(self):
        ...
        `WeightCache.verify_checksum`, which until now had none -- and it must
        pass a NON-None expected hash, since that function returns True for
        None and would otherwise pass by default.
        """
        ...
        with tempfile.TemporaryDirectory() as tmp:
            path, name = self._blob(tmp, b"the real weights")
            art = resolve_model_artifact(path)
            with mock.patch.object(WeightCache, "verify_checksum", return_value=True) as spy:
                verify_model_artifact(art)
            self.assertTrue(spy.called, "verification must go through WeightCache.verify_checksum")
