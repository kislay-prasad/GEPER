...
class TestFix1VepCsqParsing:
    ...
    def test_extract_csq_scores_missense(self):
        ...
        csq_fields = [
            ...
            "CADD_PHRED", "REVEL", "SpliceAI_pred", "AM_PATHOGENICITY",
        ]
        # values for each field
        vals = ["G", "missense_variant", "HIGH", "BRCA1", "1234",
                "Transcript", "NM_007294.4", "protein_coding",
                "5", ".", "c.5266dupC", "p.Gln1756ProfsTer74",
                "5444", "5266", "1756", "Q/X", "Caa/Xaa",
                "rs80357906", ".", "-1", ".", "SNV", "HGNC", "1100",
                "28.4", "0.92", "0.85", "0.98"]
        csq_value = "|".join(vals)
        ...
        assert spliceai == pytest.approx(0.85)  # max of the 4 delta scores
...
class TestFix2PS1:
    ...
    def test_ps1_does_not_fire_different_aa(self):
        ...
        same_aa, novel_aa = lkp.check_same_codon_pathogenic(
            ...
            wildtype_aa="Val", mutant_aa="Glu",
        )
        assert same_aa is None, "PS1 must NOT fire when AA changes differ"
...
class TestFix4UTRAnnotation:
    ...
    def _add_transcript(self, ra, tid, strand, exons, cds_start, cds_end):
        ...
        all_ends = [e for s, e in exons]
        rec = TranscriptRecord(
            transcript_id=tid, gene_id="G1", chrom="chr2",
            start=min(all_starts), end=max(all_ends),
            strand=strand, exons=sorted(exons),
    ...
    def test_map_consequence_utr5(self):
        ...
        result = _map_consequence(
            region="utr5", ref="A", alt="G",
            codon_provider=NoCdsCodonContextProvider(),
            chrom="chr2", pos=150, transcript_id="T1",
        )
    ...
    def test_map_consequence_utr3(self):
        ...
        from pipeline.annotation.stage import NoCdsCodonContextProvider
        result = _map_consequence(
            region="utr3", ref="A", alt="G",
            codon_provider=NoCdsCodonContextProvider(),
            chrom="chr2", pos=480, transcript_id="T1",
        )
...
class TestFix5GFF3Phase:
    ...
    def test_zero_phase_single_cds(self):
        ...
        ra = self._make_analyser()
        rec = TranscriptRecord(
            transcript_id="T1", gene_id="G", chrom="chr1",
            start=100, end=200, strand="+",
            exons=[(100, 200)],
            cds_records=[(100, 200, 0)],
            cds_start=100, cds_end=200,
        )
    ...
    def test_nonzero_phase(self):
        ...
        ra = self._make_analyser()
        rec = TranscriptRecord(
            transcript_id="T2", gene_id="G", chrom="chr1",
            start=100, end=200, strand="+",
            exons=[(100, 200)],
    ...
    def test_out_of_cds_returns_none(self):
        ...
        ra = self._make_analyser()
        rec = TranscriptRecord(
            transcript_id="T3", gene_id="G", chrom="chr1",
            start=100, end=200, strand="+",
            exons=[(100, 200)],
            cds_records=[(120, 180, 0)],
            cds_start=120, cds_end=180,
        )
...
class TestFix6CanonicalTranscript:
    ...
    def test_mane_select_preferred_over_other(self):
        ...
        rec_mane = GeneRecord(
            ...
            gene_name="BRCA1", gene_id="HGNC:1100",
            transcript_id="NM_007294.4", feature_type="mRNA",
            is_mane_select=True,
        )
        rec_other = GeneRecord(
            chrom="chr1", start=1000, end=2000,
            gene_name="BRCA1", gene_id="HGNC:1100",
            transcript_id="NM_007297.3", feature_type="mRNA",
    ...
    def test_mane_plus_clinical_preferred_over_non_canonical(self):
        ...
        rec_mpc = GeneRecord(
            chrom="chr1", start=1000, end=2000,
            gene_name="BRCA1", gene_id="HGNC:1100",
            transcript_id="NM_007300.4", feature_type="mRNA",
            is_mane_plus_clinical=True,
        )
        rec_plain = GeneRecord(
            chrom="chr1", start=1000, end=2000,
            gene_name="BRCA1", gene_id="HGNC:1100",
            transcript_id="NM_999999.1", feature_type="mRNA",
        )
    ...
    def test_longest_cds_tiebreaking(self):
        ...
        from pipeline.annotation.gff_index import GeneRecord
        short = GeneRecord(
            chrom="chr2", start=100, end=500,
            gene_name="GENE2", gene_id="G2",
            transcript_id="NM_short.1", feature_type="mRNA",
            cds_length=200,
        )
        long_ = GeneRecord(
            chrom="chr2", start=100, end=500,
            gene_name="GENE2", gene_id="G2",
            transcript_id="NM_long.1", feature_type="mRNA",
            cds_length=800,
        )
...
class TestFix9ACMGConflict:
    ...
    def test_conflict_strong_path_plus_strong_benign(self):
        ...
        classification, explanation = self._run_classify([
            {"code": "PS1", "met": True, "direction": "pathogenic", "strength": "strong", "reason": ""},
            {"code": "BS1", "met": True, "direction": "benign", "strength": "strong", "reason": ""},
        ])
    ...
    def test_conflict_pvs_plus_ba(self):
        ...
        classification, explanation = self._run_classify([
            {"code": "PVS1", "met": True, "direction": "pathogenic", "strength": "very_strong", "reason": ""},
            {"code": "BA1",  "met": True, "direction": "benign", "strength": "stand_alone", "reason": ""},
        ])
        assert classification == "Uncertain_Significance"
        assert "Conflicting_Evidence" in explanation or "conflicting" in explanation.lower()
...
class TestFix10DeadCode:
    ...
    def test_aggregator_delegates_to_lookup(self):
        ...
        assert EvidenceAggregator.clinvar_sig_to_score("Pathogenic", 4) == \
               ClinVarLookup.sig_to_score("Pathogenic", 4)
        assert EvidenceAggregator.clinvar_sig_to_score("Benign", 2) == \
               ClinVarLookup.sig_to_score("Benign", 2)
        assert EvidenceAggregator.clinvar_sig_to_score("Uncertain significance", 0) == \
...
class TestFix11LargeGene:
    ...
    def _build_large_gene_index(self):
        ...
        gene_rec = GeneRecord(
            chrom="chrX", start=31_000_000, end=33_500_000,
            gene_name="DMD", gene_id="HGNC:2928",
            transcript_id="", feature_type="gene",
        )
        tx_rec = GeneRecord(
            chrom="chrX", start=31_000_000, end=33_500_000,
            gene_name="DMD", gene_id="HGNC:2928",
            transcript_id="NM_004006.3", feature_type="mRNA",
        )
...
class TestEdgeCases:
    ...
    def test_vep_csq_score_propagation_end_to_end(self):
        ...
        try:
            ...
            assert v.alphamissense_score == pytest.approx(0.87), "AlphaMissense not propagated from CSQ"
        finally:
            os.unlink(path)
