# GEPER — External Bioinformatics Tool Installation Guide

GEPER's core pipeline (FASTQ → Alignment → Variant Calling → Annotation →
...
| `vep`       | Optional  | External Ensembl VEP annotation (GEPER degrades gracefully without it) |

Before running the pipeline, GEPER performs a single consolidated
...
**required** tool is missing, GEPER fails immediately with one clear
...
Using Docker isolates GEPER from host-level version drift and is the
...
Native Windows binaries for these tools are not maintained; run GEPER
...
3. Run GEPER entirely from within the WSL2 shell — do not attempt to mix
...
VEP is optional; GEPER runs without it (gene-dependent ACMG criteria such
