"""
Caching for HPO gene-phenotype evidence lookups.

Same two-layer, independently-toggleable shape as
`pipeline/clingen/cache.py::ClinGenCache` (in-process bounded/TTL'd LRU,
...
HPO's gene-phenotype annotation changes on the order of its periodic
releases (roughly monthly), so this defaults to the same long TTL as
ClinGen's gene-level cache (24h), configurable via
`CONFIG.hpo.CACHE_TTL_SECS`.
"""
...
class HPOCache:
    """Thread-safe, bounded, TTL'd LRU cache of HPO gene-evidence lookup results (as plain dicts)."""
    ...
    def _append_to_disk(self, key: str, value: Dict[str, Any]) -> None:
        try:
            ...
            # lookups (same policy as ClinGenCache/GnomadCache).
            logger.warning(f"Could not persist HPO cache entry to '{self.disk_path}': {exc}")
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Could not read HPO disk cache '{self.disk_path}': {exc}")
            return
        if loaded:
            logger.info(f"Loaded {loaded} cached HPO entr{'y' if loaded == 1 else 'ies'} from '{self.disk_path}'.")
            with self._lock:
                while len(self._store) > self.max_size:
                    self._store.popitem(last=False)
