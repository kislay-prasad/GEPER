...
# Paper: Jonsson et al., "Transformers significantly improve splice site
#   prediction", Communications Biology 7, 1-9 (2024).
#   https://doi.org/10.1038/s42003-024-07298-9
# Fetched and vendored unmodified below this header comment block (the
# module docstring under `keras_init` import onward is upstream's own
# code, including its own inline comments/commented-out code -- left
# exactly as published so this is a verifiable, diffable copy of the
# real upstream file, not a paraphrase). See this package's __init__.py
# for GEPER's vendoring policy. Do not hand-edit; re-vendor from
# upstream instead.
#
# GEPER uses only the `SpliceFormer` class from this file (instantiated
# with the same hyperparameters the official repo's own inference/
# delta-scoring notebook uses -- see pipeline/models/spliceformer_plugin.py
# for exactly which ones and where they were sourced from). The other
# classes in this file (`SpliceAI_10K`, `SpliceAI_small`, `ResNet_40K`)
# are alternative/ablation architectures from the same paper that
# GEPER's plugin does not load or reference.
# =============================================================================
# +
import torch
import numpy as np
